import {
    ...
    AnalyzedAssetBundle,
    Platform,
} from '../utils/assetbundle';
import { NestLogger } from '../utils/logger';

export { checkValidAssetFile };

const logger = new NestLogger();
const CTX = 'AvatarUtils';

export interface AvatarDescriptorInfo {
    platform: string;
    engine: string;
    descriptorCount: number;
}

const AVATAR_DESCRIPTOR_NAMESPACE = 'Nox.CCK.Avatars';
const AVATAR_DESCRIPTOR_CLASS = 'AvatarDescriptor';

/**
 * Validates the analyzed bundle for an avatar asset:
 * - Engine must be "unity"
 * - Platform in bundle must match expected platform
 * - Engine in bundle must match expected engine
 * - At least one AvatarDescriptor MonoBehaviour must exist
 *
 * Throws with a descriptive message on any mismatch.
 */
export function analyzeAvatarAsset(
    data: AnalyzedAssetBundle,
    expectedEngine: string,
    expectedPlatform: string,
): AvatarDescriptorInfo {
    logger.debug(`Analyzing avatar asset — engine: ${data.engine}, platform: ${data.platform}, name: ${data.name}`, CTX);
    ...
    for (const mb of data.data.MonoBehaviour ?? []) {
        ...
        if (cls?.namespace === AVATAR_DESCRIPTOR_NAMESPACE && cls?.class === AVATAR_DESCRIPTOR_CLASS)
            descriptorCount++;
    }
    logger.debug(`AvatarDescriptor count: ${descriptorCount}`, CTX);

    if (descriptorCount === 0)
        throw new Error('No AvatarDescriptor found in asset bundle');
    ...
    logger.debug(`Avatar asset validated — platform: ${bundlePlatform}, engine: ${data.engine}, descriptors: ${descriptorCount}`, CTX);
